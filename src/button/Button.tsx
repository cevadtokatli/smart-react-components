...
const Button: React.FC<Props> = ({size="default",sizeSm,sizeMd,sizeLg,sizeXl,elementProps=DV.JSX_ELEMENT_PROPS,children,type,fill,link,border,shape,block,fixedSize,waveEffect="light",icon,iconPosition="left",loading,loadingStatus,loadingTransitionClassName="src-button-loading",loadingTransitionType="ease-in-out",loadingTransitionDuration=300,loadingTransitionShowAnimation=true,loadingTransitionHideAnimation=true}) => {
    ...
    const ButtonEl = (
        <ButtonElement {...elementProps} size$={size} size$Sm={sizeSm} size$Md={sizeMd} size$Lg={sizeLg} size$Xl={sizeXl} type$={type} fill$={fill} link={link} border$={border} shape={shape} block={block} fixedSize={fixedSize} loading$={loadingStatus}>
